...
from models import Holding, HoldingStatus, Platform, ResearchReport, User, cost_basis, market_value, profit
...
def create_run(
    ...
    # 2. Fail fast if AI is not configured
    provider = ai_client.normalize_provider(ai_provider)
    model = ai_client.choose_model(provider, ai_model)
    ...
    # 6. Build prompt using AI Berkshire skill + platform context
    ...
    # 8. Persist (status=queued, snapshot skill_md for reproducibility)
