...
## [功能] - 2026-09-24 现金账户与流水、买卖表单持仓提示、移动端盈亏日历

### 类型：✨新增

- **改了什么**：
  1. **现金账户**：新增按「平台 + 币种」管理的现金账本。入金/出金/分红/利息/现金校准（`cash_adjust`）与买卖联动：买卖按「量×价 ± 费」计现金流，手填金额按实际净额（不重复计费）；`cash_adjust` 填写该时点现金**绝对余额**，建立起始基准。账户详情页新增现金区（初始化/校准、入金、出金、流水），交易表单显示「本次现金增减 + 预计交易后余额」。总资产 = 持仓市值 + 现金 + 其它资产，现金变动不污染每日盈亏。
  2. **买卖表单持仓提示**：新增「选择已有持仓」下拉（名称/代码/账户/币种可搜索），自动带出平台/代码/名称/币种，显示「当前持仓 X 股」与「交易后 X 股」，卖出支持「全部卖出」；按（平台+代码+币种）精确匹配，区分新标的 0 股 / 数量未知 / 信息不足。
  3. **移动端盈亏日历**：每日盈亏抽屉在手机端默认显示 7 列月历（原为列表），支持上/下月/回到本月、已记录天数合计与「数据不完整」标注；格子区分零 / 无记录 / 数据不足 / 基准 / 待更新 / 未来，高亮今天与已选日，返回保留月份与视图。
- **为什么**：现金是总资产的重要组成，此前只能作为手填持仓维护、无法随交易自动增减；买卖时看不到当前持仓数量易出错；手机端盈亏抽屉默认列表不符合月历阅读习惯。
- **影响范围**：`backend/cash_service.py`（新增）、`backend/routers/cash.py`（新增）、`backend/routers/transactions.py`、`backend/transaction_validation.py`、`backend/importers/generic.py`、`backend/models.py`、`backend/database.py`、`backend/import_service.py`、`backend/tests/test_cash.py`（新增）、`frontend/src/cash.js`（新增）、`frontend/src/pages/Transactions.jsx`、`frontend/src/pages/PlatformDetail.jsx`、`frontend/src/components/DailyPnlDrawer.jsx`、`frontend/src/api/index.js`、`frontend/src/constants.js`。
- **注意事项**：无破坏性数据库结构变更（现金为 `derived` 持仓，随流水重算）。现金须先「初始化/校准」建立基准，之后买卖才计入现金；未初始化账户的买卖不虚构现金。出金/买入导致现金为负会被按时间线拒绝（不做 `max(balance,0)` 掩盖）。已验证后端全量 368 项测试通过、前端构建通过。

## [功能] - 2026-09-24 持仓校准与无买入记录卖出

- **改了什么**：已有按数量维护的手填持仓可直接记录卖出，系统保留期初校准记录并自动扣减数量。持仓列表及交易页新增「持仓校准」，直接填写总数量、平均成本和日期，无需补录逐笔历史买入。
- **收益口径**：校准不产生现金流或已实现收益；成本未知的卖出标记「待补充成本」，补齐原校准记录后重算。校准当天重新建立每日盈亏基准，避免将校准差额记作收益。
- **一致性**：买卖、校准的修改/删除按日期及同日录入顺序校验；备份恢复及 CSV 导入支持校准，普通 CSV 导入失败整批回滚。同一平台、代码、币种有重复持仓时提示先整理。
- **注意事项**：重启后端自动补充 `holding.realized_pnl_incomplete` 列及 PostgreSQL 交易枚举；无须改动现有账本。校准数量是当时总持仓，不是新增数量；手填总市值资产仍按金额维护。

## [功能] - 2026-09-16 逐仓位每日盈亏明细 + 币种切换联动

### 类型：✨新增

- **改了什么**：
  1. 每日盈亏抽屉新增「逐仓位明细」：桌面点击日历日期、列表点「查看明细」即可查看当天每个平台/账户下各持仓的盈亏；同一股票在不同平台的仓位分别展示，默认按盈亏绝对值降序。移动端无需悬停，列表直接提供「查看明细」按钮。
  2. 后端 `DailyPnl` 新增 `details_json` 字段（旧表启动自动补列），保存逐仓位历史快照（平台/账户名、代码、名称、原币种、累计收益组成、当日原币/CNY/USD 金额、当次汇率与时间、状态及原因）；新增 `GET /api/snapshots/daily-pnl/{day}?currency=` 按日查询接口。
  3. 每日盈亏抽屉复用 `DisplaySettingsContext` 的 `displayCurrency`，并新增抽屉内人民币/美元切换入口；日历、Tooltip、列表、当月合计、详情总额、逐仓位金额全部随全局币种联动，金额由后端按记录时汇率换算、前端不再二次换算。
- **为什么**：用户需要知道「哪一天、哪个平台、哪只股票赚/亏多少」，且切换人民币/美元后所有盈亏金额与币种标识必须一致、不能只换符号。
- **影响范围**：`backend/models.py`、`backend/database.py`、`backend/daily_pnl_service.py`、`backend/routers/snapshots.py`、`backend/routers/backup.py`、`backend/tests/test_daily_pnl_detail.py`（新增）、`frontend/src/api/index.js`、`frontend/src/components/DailyPnlDrawer.jsx`、`frontend/src/pages/Dashboard.jsx`、`docs/daily-pnl-and-portfolio-analysis.md`。
- **注意事项**：
  - 数据库结构变更（`dailypnl` 新增 `details_json` 列），重启即自动迁移，无需重装依赖。
  - 历史明细自本版本开始记录：此前已有每日记录只有汇总、无逐仓位明细，查看时提示「该日期未记录持仓明细」。
  - 逐仓位金额各自四舍五入到分，与当日总额可能存在每仓位最多 0.5 分的舍入差（接口 `rounding_diff` 标注）。
  - 「今日涨跌」（当日价格变动）与「每日盈亏」（累计收益日变化）口径不同，数值通常不相等，已在界面与文档标注，请勿混用。
  - 已验证：后端全量 321 项测试通过、前端 `npm run build` 通过、旧表补列迁移通过。本次仅本地实现，未部署。

## [修复] - 2026-09-12 每日盈亏容错：无法估值持仓不再拖垮整日统计

### 类型：🐛修复 / ⚡优化

- **改了什么**：`daily_pnl_service.py` 把「算不出盈亏（缺价格或成本）的持仓」从「拦截整天、标数据不足」改为「自动排除、照常计算」——能估值的持仓正常算日差额，无法估值的跳过，并在当天结果标注「N 项持仓缺少价格或成本，未计入」；某持仓在「能估值/不能估值」之间切换（或持仓增减、本金变动）时自动重建基准，避免把这种切换误当成当日盈亏。
- **为什么**：场外基金抓不到净值时（如香港基金 `HK0000720752`，东方财富基金库只覆盖内地基金）`current_price` 一直为空，导致整天每日盈亏都显示「数据不足」。现金本已排除、不受影响，但基金及未来其它抓不到价的资产仍会拖垮整日，故改为容错排除。
- **影响范围**：`backend/daily_pnl_service.py`（排除逻辑 + 基准口径改为「非现金且可估值」）、`backend/tests/test_daily_pnl.py`（新增 1 项「无法估值持仓不拦截」测试，并更新价格缺失/清仓/行情过期场景断言）。
- **注意事项**：无数据库结构变更，重启即生效；已验证后端全量 310 项测试通过。货币基金（净值≈1）建议改「手填市值 + 投入成本」，或把利息记为「分红/利息」交易，让每日计息进入盈亏。
...
- **改了什么**：删除旧的 `backend/.venv/`、根目录 PPT 专用 `node_modules/`、`package.json`、`package-lock.json`，以及后端 Python 字节码和 pytest 缓存；按清理前磁盘分配量合计约 2.64 GiB。
- **双平台保留**：保留 `.venv-mac-py39/` 与 `.venv-win-py311/`，继续由 `dev.py` 按系统和 Python 版本选择环境；保留 Windows、macOS 和 Shell 启停脚本。
- **忽略规则**：`.gitignore` 统一忽略各层级 `node_modules/` 和 `.pytest_cache/`；`.dockerignore` 补齐 `.venv-*`、pytest 缓存及各层级 Node 依赖，避免本地环境被上传到构建上下文或复制进镜像。
- **影响范围**：仅清理可重建的环境、遗留 PPT 依赖与缓存；账户数据库、环境配置、业务代码、前端依赖及构建产物均保留。缓存可能在运行中再次生成。
- **开发说明**：Python 环境按平台独立维护；切换系统时不要共用前端安装产物，必要时在 `frontend/` 执行 `npm ci` 重建。
- **清理验收**：Mac 后端核心依赖可导入，后端健康接口返回正常，前端返回 HTTP 200；未改动业务逻辑，本次未重复运行完整测试。

## [功能] - 2026-09-12 每日盈亏、登录刷新与全账户 AI 分析完善

### 类型：✨新增 / 🐛修复 / ⚡优化

- **每日盈亏**：总览「今日涨跌」旁新增入口，支持近 30 天、90 天、一年；桌面表格、手机列表与隐私打码。新表 `dailypnl` 按 UTC 日记录累计持仓收益差额，包含已实现收益和分红，排除入出金与汇兑变动；缺基准、缺价格或成本、行情过期、账本口径变化时显示原因，不伪造历史收益。
- **自动刷新**：登录或恢复已登录页面时共享一次行情、汇率和盈亏刷新任务，支持失败提示与重试。后台全量任务同时记录每日盈亏；Fly 配置为北京时间 07:55 每日刷新，启动时刷新，并保留一个运行实例。线上部署尚未完成，需有效 Fly.io 登录态及发布后核验，不能将配置完成等同于线上已生效。
- **盈亏与隔离修复**：缺少现价不再计算为全额亏损，清仓持仓不再计入当前市值；自动化记录补充用户归属并按用户过滤，部分刷新失败不再显示为全部成功。
- **资产走势**：支持最大、1 年、半年、3 个月、1 个月、1 周，首次默认半年，按用户记忆选择；过滤无效和未来日期，处理空区间、单点及乱序请求，优化移动端选择器布局。
- **全账户 AI**：自动使用用户默认 Provider、模型和 Key，保留手动覆盖；显示输入数据时点及币种，新任务状态与旧成功摘要分开。补充无有效资产、上下文超限、重复提交和失败重试处理，行动项去重并在可靠识别时关联持仓。
- **报告与界面**：新组合报告采用固定八节结构、三条摘要、持仓建议表及行动清单，规范币种、精度、事实与未知的表达；摘要渲染 Markdown，拓宽完整报告阅读区域，隐私模式折叠摘要。移除首页顶部重复的「AI 分析全部账户」蓝色按钮，保留投研页面和摘要卡片入口。
- **数据兼容**：启动时创建日盈亏表并给旧 `automationrun` 表补充 `user_id`；备份 v2 包含日盈亏及资产快照，兼容旧格式，恢复后重新建立计算基准。
- **验证与说明**：功能开发阶段后端全量 303 项测试通过，前端生产构建通过（仍有大包体积提示），模拟 API 的桌面及手机浏览器验收通过；后续投研变更的相关 55 项测试通过。未进行真实付费 AI 调用。详见 [每日盈亏与组合分析](docs/daily-pnl-and-portfolio-analysis.md)、[组合报告 Prompt](docs/portfolio-report-prompt.md)。
